...
function renderArticle() {
  ...
  const html = `<a href="javascript:history.back()" class="back-link">← Torna alle news</a><article class="article-detail"><div class="article-detail-header"><span class="cat-badge" style="background:${catInfo.color || '#C0392B'}">${catInfo.emoji || ''} ${article.category}</span><h1>${escapeHtml(article.title)}</h1><div class="meta"><span>📅 ${formatDate(article.date)}</span><span>📰 ${escapeHtml(article.source || 'Fonte sconosciuta')}</span></div></div><div class="article-detail-body"><div class="abstract">${escapeHtml(article.abstract)}</div><div class="full-content"><p>${escapeHtml(article.content)}</p>${article.url ? `<p><a href="${escapeHtml(article.url)}" target="_blank" rel="noopener" style="color:#C0392B;font-weight:600;">Leggi la fonte originale →</a></p>` : ''}</div><div class="article-source-box"><div><div class="label">Fonte</div><div class="source-name">${escapeHtml(article.source || 'Fonte sconosciuta')}</div></div>${article.url ? `<a href="${escapeHtml(article.url)}" target="_blank" rel="noopener" style="background:#C0392B;color:white;padding:8px 20px;border-radius:6px;text-decoration:none;font-weight:600;font-size:14px;">Visita</a>` : ''}</div></div></article>`;
  main.innerHTML = html;
}
